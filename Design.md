...
| 27 | How do the Webhook Sender and HTTP Adapter know which external endpoint to call? | **One fixed target per instance** (`JETCORE_WEBHOOK_TARGET_URL` / `JETCORE_HTTP_TARGET_BASE_URL`, plus an optional outbound auth header/secret each) — the same per-instance-identity pattern Decision #18 already established for horizontal scaling (multiple targets = multiple adapter instances, each its own NATS identity/config, not one adapter dynamically routing by message content). |
...
- [ ] **F1. Schemas** — `schemas/events.files.FileReadRequested.v1.json`/`FileReadCompleted`, `FileListRequested`/`FileListCompleted`, `FileDeleteRequested`/`FileDeleteCompleted`, and one shared `FileOperationFailed.v1.json` (Decision #23), mirroring Step C3's approach and the existing write-path schemas' style exactly.
- [ ] **F2. `read_handler.py`** — subscribes to `FileReadRequested` (`{"path"}`), resolves via the existing `resolve_within` (Step C4, reused as-is), publishes `FileReadCompleted` (`{"path", "content" (base64), "sizeBytes"}`) or `FileOperationFailed` (`{"path", "operation": "read", "reason": "not_found"}`) if the target doesn't exist. Traversal/malformed input: silent ack+log, same posture as the write path.
- [ ] **F3. `list_handler.py`** — subscribes to `FileListRequested` (`{"path"}`, the directory to list — empty string means `watch_dir` itself), publishes `FileListCompleted` (`{"path", "entries": [{"name", "isDirectory", "sizeBytes"}]}`), non-recursive (Decision #23). `FileOperationFailed` if `path` doesn't exist or isn't a directory.
- [ ] **F4. `delete_handler.py`** — subscribes to `FileDeleteRequested` (`{"path"}`), publishes `FileDeleteCompleted` (`{"path"}`) or `FileOperationFailed` (`{"path", "operation": "delete", "reason": "not_found"}`).
- [ ] **F5. Manifest + entrypoint wiring** — [infra/nats/adapter_identities.yaml](infra/nats/adapter_identities.yaml): add `FileReadCompleted`/`FileListCompleted`/`FileDeleteCompleted`/`FileOperationFailed` to `file-storage-01`'s publish list (the `Requested` subjects are already there from Step A2); re-run `bootstrap_auth.sh`. [`__main__.py`](adapters/file_storage_adapter/file_storage_adapter/__main__.py)'s `run()` grows from a 2-way `asyncio.gather` (write handler + external-creation watch, Step C6) to a 5-way one — each command type is its own durable consumer, per §6's "one filtered consumer per adapter" design (not one consumer juggling four subjects).
- [ ] **F6. Tests + verification** — unit tests per handler (mirroring C4's `test_write_handler.py` structure: happy path, not-found, traversal-rejected), plus an entrypoint-level test extending Step C7's pattern to cover all four commands running together. `ruff`/`mypy`/`bandit`/`pip-audit` clean.

### Track G — Webhook Sender

- [ ] **G1. Package scaffold** — `adapters/webhook_sender/pyproject.toml` (`jetcore` + `httpx`, per §7.2), package dir, `tests/` with a uniquely-named scaffold test from the start (Step C1's lesson applied proactively, not rediscovered).
- [ ] **G2. `settings.py`** — `WebhookSenderSettings(AdapterSettings)` adding `target_url: str` and an optional `outbound_secret: SecretStr | None` (Decision #27) — sent as `X-Webhook-Secret` on the outbound POST if configured, the mirror image of the Webhook Listener's own inbound check (Decision #21).
- [ ] **G3. Relay handler** — subscribes to `JETCORE_SUBJECTS` (already a generic `AdapterSettings` field, unused until now — every prior adapter's subscribe list was hardcoded), POSTs each decrypted payload to `target_url` via `httpx`, **single attempt, no retry** (Decision #12) — acks regardless of the HTTP outcome (nak/redelivery would itself be a retry, contradicting "best-effort"), logs failures.
- [ ] **G4. Entrypoint** — `__main__.py`, no HTTP server needed (outbound-only), matching the shape of a pure background consumer (closer to the File Storage Adapter's write handler than to the Webhook Listener's FastAPI app).
- [ ] **G5. Tests + verification** — relay-on-receipt, secret-header-sent-when-configured, failed-POST-still-acks (a local test HTTP server that returns 500, confirming no redelivery afterward — reusing `BusClient.fetch()`'s idle-returns-`[]` fix from Step C4 for exactly this check, same idiom as Steps C4/C5's own tests).

### Track H — HTTP Adapter

- [ ] **H1. Package scaffold** — `adapters/http_adapter/pyproject.toml` (`jetcore` + `httpx`).
- [ ] **H2. `settings.py`** — `HttpAdapterSettings(AdapterSettings)` adding `target_base_url: str` and an optional outbound auth header/token (Decision #27).
- [ ] **H3. Trigger handler** — subscribes to `JETCORE_SUBJECTS` (the configured trigger subject, e.g. the placeholder `events.orders.OrderCreated`), calls `target_base_url` via `httpx`, publishes `events.http.RequestCompleted` (`{"status": "success"|"error", "statusCode", "body"}` — `body` base64, since an arbitrary external API's response isn't guaranteed to be JSON or even text) with `correlationId` set to the triggering event's `eventId`.
- [ ] **H4. Entrypoint.**
- [ ] **H5. Tests + verification** — trigger-to-call mapping, success and error status codes both produce a `RequestCompleted` (this adapter's job is reporting the outcome, not judging it), against a local test HTTP server.

### Track I — REST API Service

- [ ] **I1. `jetcore` change: `BusClient.publish()` returns the generated `eventId`** (Decision #24) — the one piece every other track in this phase doesn't need but this one can't work without. Small, additive, backward compatible (existing callers that ignore the return value are unaffected); update [libs/jetcore/tests/test_bus_client.py](libs/jetcore/tests/test_bus_client.py) to assert on it.
- [ ] **I2. Package scaffold** — `adapters/rest_api_service/pyproject.toml` (`jetcore` + `fastapi` + `uvicorn`).
- [ ] **I3. `settings.py`** — `RestApiServiceSettings(AdapterSettings)` adding `http_port` and a `default_reply_timeout_seconds` (the cap on how long `?wait=` is allowed to block).
- [ ] **I4. Pending-reply correlation** — `PendingReplies`, an in-memory `dict[str, asyncio.Future[ReceivedEvent]]` keyed by `correlationId`, fed by a background durable-consumer loop on `events.orders.OrderPersisted` (Decision #26) that resolves a matching future per arrival (and drops/logs one with no matching pending request — a reply that outlived its own timeout).
- [ ] **I5. HTTP endpoint** — `POST /api/orders` (the placeholder bounded context, Decision #14): publishes `events.orders.OrderCreated`, captures the returned `eventId` (Step I1) as `correlationId`. No `?wait=` query param (or `?wait=0`): `202 Accepted` immediately, matching the Webhook Listener's own fire-and-forget default. `?wait=<seconds>` (capped at `default_reply_timeout_seconds`): registers a pending reply, awaits it, returns `200` + the `OrderPersisted` payload, or `504` on timeout (unregistering the pending entry either way, so a late reply after timeout doesn't leak a resolved-but-unread future).
- [ ] **I6. Entrypoint.**
- [ ] **I7. Tests + verification** — fire-and-forget path (D5's pattern: real `create_app`, no injected client), the sync-reply path against a **fake replier** (a test-only client that publishes a matching `OrderPersisted` on cue, so this track's own tests don't need the real Database Adapter to exist yet), and a genuine timeout case (nothing ever replies) asserting `504`.

### Track J — Database Adapter (MySQL)

The only track needing new infra.

- [ ] **J1. MySQL infra** — `docker-compose.yml`'s `mysql` service (§7.3): binlog enabled (`log_bin`, `binlog_format=ROW`, unique `server-id`) via an `infra/mysql/my.cnf` mount; `infra/mysql/init.sql` creates the demo `orders` table (Decision #25) and the two MySQL users (Decision #25/Track parameters: one DML user for the write path, one `REPLICATION SLAVE`/`REPLICATION CLIENT`-only user for CDC — least privilege, matching the mitigation already recorded in [Dependencies.md](Dependencies.md)).
- [ ] **J2. Package scaffold** — `adapters/db_adapter_mysql/pyproject.toml` (`jetcore` + `SQLAlchemy[asyncio]` + `asyncmy` + `python-mysql-replication`, per §7.2).
- [ ] **J3. `settings.py`** — `DbAdapterSettings(AdapterSettings)` adding the write-path DB connection (host/port/user/password/database) and the *separate* CDC user's credentials — two distinct identities, not one set of credentials reused for both roles.
- [ ] **J4. Write path** — subscribes to `events.orders.OrderCreated`, `INSERT ... ON DUPLICATE KEY UPDATE` (upsert) keyed on `order_id` (Decision #25 — no round trip needed to learn a DB-assigned id), publishes `events.orders.OrderPersisted` (Decision #26) with `correlationId` = the request's `eventId`.
- [ ] **J5. CDC read path** — `python-mysql-replication` tailing the binlog for the `orders` table (Decision #15, already settled), translating `WriteRowsEvent`/`UpdateRowsEvent`/`DeleteRowsEvent` into the shared CDC shape (Track parameters table), publishing to `events.db.orders.RowChanged` — deliberately a different subject from the write path's input (already reflected in the Step A2 manifest, so the adapter can never subscribe to its own CDC output).
- [ ] **J6. Entrypoint** — write handler + CDC watcher running concurrently (`asyncio.gather`, the same two-independent-pieces shape as the File Storage Adapter's write handler + external-creation watch, Step C6).
- [ ] **J7. Tests + verification** — needs a **real MySQL**, not mocked (matching this whole project's practice for NATS): either the `docker-compose` `mysql` service directly (this project's established pattern for NATS throughout Phases 1-2) or `testcontainers-python` (already earmarked in §7.4/Dependencies.md — this is the track that would actually adopt it, if the docker-compose service alone proves awkward for test isolation/reset between runs). Write-path upsert, CDC detects a bus-originated write **and** a direct SQL write (proving it's really tailing the binlog, not just echoing its own writes), `OrderPersisted` correlation.

### Track K — Integration

- [ ] **K1. Adapter `Dockerfile`s** — four more, mirroring [adapters/file_storage_adapter/Dockerfile](adapters/file_storage_adapter/Dockerfile)/[adapters/webhook_listener/Dockerfile](adapters/webhook_listener/Dockerfile) exactly (Step E2's pattern: repo-root build context, every workspace member's `pyproject.toml` present, non-root `appuser`).
- [ ] **K2. `docker-compose.yml` wiring** — the `mysql` service (Track J) plus four adapter services, each's real `.creds` file mounted, `JETCORE_*` env vars per §7.6, `restart: unless-stopped` (Step E1's chosen mechanism for "dependency wasn't ready yet," reused rather than reconsidered).
- [ ] **K3. Manual end-to-end smoke test** — same spirit as Steps A7/E3, through the real stack: File Storage read/list/delete via direct bus publish (no HTTP front door for these yet); Webhook Sender relaying to a local `python -m http.server`-style receiver; HTTP Adapter calling a local test REST endpoint; REST API Service's `?wait=` round trip against the *real* Database Adapter; and — the one check that actually proves the CDC path is real — a direct `mysql` client `INSERT` bypassing the bus entirely, confirming `events.db.orders.RowChanged` still fires.
- [ ] **K4. Automated end-to-end tests** — one cross-adapter test for the REST API Service ↔ Database Adapter round trip (the genuinely new "two real adapters, one waiting on the other's reply" pattern this phase introduces), in the root [tests/](tests/) directory alongside Step E4's — File Storage's four commands are already covered at the adapter level by Track F's own tests, and Webhook Sender/HTTP Adapter are one-way enough that Track G/H's own adapter-level tests are sufficient without a dedicated cross-adapter test.

**Exit criteria:** each track passes standing alone first (mirroring Phase 2's own exit criteria), then K3+K4 both pass against the fully wired `docker-compose` stack, MySQL included.
