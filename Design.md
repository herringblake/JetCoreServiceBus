...
- [ ] **B7. End-to-end proof** — a test with two in-process fake adapters using `BusClient`: one publishes a signed+encrypted event, the other receives, verifies the signature, and decrypts it. This is the concrete "Phase 1 is done" checkpoint — everything downstream (real adapters) builds on this working. **Note: B6's own integration tests already do most of this** (real publish → real JetStream delivery → real decrypt+verify between two real BusClient instances) — B7 may end up light, more a confirmation/consolidation than new ground.
